...
for(i in 1:9){
  ...
  WSE30mat10m<- flood.30mat10m$max[keepInds] + elev.30mat10m$norristown_30m_new[keepInds]
  ...
  WSH30mat10m<- flood.30mat10m$max[keepInds] + 
  ...
  z1= matrix(vals.30m$max, nrow= ny30m, ncol= nx30m, byrow= TRUE)
  ...
  downscale30m<- rep(NA,length(flood.10m$max))
  ...
  print(ggplot(residsbyloc.df, aes(x, y, color=value))+
          geom_point(shape="square",size=1.8) + ggtitle(paste0("Q",quantiles[i],"- Residuals in space- WSHtocompare~ downscale30m")) +theme_bw()+
  ...
  save(residsbyloc.df,file=paste0("C:/Users/svr5482/Reification/Philly/data/Norristown/nCh/simplecal/WSH10-downscale30_q",quantiles[i],".RData"))
  ...
  jpeg(filename=paste0("C:/Users/svr5482/Reification/Philly/plots/dnscl,cal30-cal10resids_spatial_q",quantiles[i],".jpeg"), width = 800, height = 700)
  print(ggplot(residsbyloc.df, aes(x, y, color=value))+
    geom_point(shape="square",size=1.8) + ggtitle(paste0("Q",quantiles[i],"- Residuals in space- WSH10m- downscale30m")) +theme_bw()+
