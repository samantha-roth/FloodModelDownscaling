...
if(dir.exists("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/calibration/30m")==F){
  dir.create("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/calibration/30m")}

if(dir.exists("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/calibration/30m/ST10")==F){
  dir.create("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/calibration/30m/ST10")}
...
load("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/closest30m_RS.RData")
closest30m_RS<- closest30m_RS[RS10inds,]

#load emulator parameters from the emulator fit with R
load("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/ST/parsFN1e-5ExpGPbyLoc.30m.RData")
kappa.vec30<- kappa.vec30[RS10inds]
phi.vec30<- phi.vec30[RS10inds]
zeta.vec30<- rep(1e-5,length(RS10inds))
...
y.c<- apply(closest30m_RS,1,function(y) y- mean(y))
mean.y<- rowMeans(closest30m_RS)
...
predMeanCov<- function(prop_ch,loc){
  ...
  zeta=zeta.vec30[loc]
  phi=phi.vec30[loc] #retrieve the estimated parameter from the matrix
  kappa=kappa.vec30[loc]
  ...
  pred30=Sigma12%*%Sig2inv[,,loc]%*%y.c[,loc] #predicted values from the emulator
  var30=diag(Sigma11-Sigma12%*%Sig2inv[,,loc]%*%t(Sigma12))
  list(pred=pred30,var=var30)
}
...
#load("/storage/work/svr5482/FloodingModelCalibrationProject/multires/spatial/changPC2014/10m30m/outputData/nCh_RWE/100e400c/just10m/cal/mcmc_res_homGP10.RData")
...
#output <- mh.alg(init = init_vals, n.sample = 300000)
...
#setwd("/storage/work/svr5482/FloodingModelCalibrationProject/multires/spatial/changPC2014/10m30m/outputData/nCh_RWE/100e400c/just10m/cal/.25/3cm")
setwd("/storage/work/svr5482/Reification/Philly/data/Norristown/nCh/calibration/30m/ST10") 
